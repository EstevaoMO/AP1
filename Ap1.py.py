...
def aventureiro_atacar(aventureiro):
    ...
    entre 1 e 6.
    """
    ataque = aventureiro["forca"]
    return(ataque + randint(1,6))
...
def aventureiro_defender(aventureiro, dano):
    ...
    """
    dano_recebido = dano - aventureiro["defesa"]
    if dano_recebido < 0:
        dano_recebido = 0
    return (aventureiro["vida"] - dano_recebido)

def ver_atributos_aventureiro(aventureiro):
    """
    Exibe na tela os atributos do aventureiro (nome, vida, força, defesa).
    """
    for stats,valores in aventureiro.items():

        print(f"{stats}: {valores}")
...
def iniciar_combate(aventureiro, monstro):
    ...
    while True:
        ...
        print(f"{aventureiro["nome"]} causou {dano_aventureiro} de dano! Vida do monstro: {monstro["vida"]}")

        if monstro["vida"] <= 0:
            return not monstro_esta_vivo(monstro)

        dano_monstro = monstro["forca"]
        aventureiro["vida"] = aventureiro_defender(aventureiro, dano_monstro)
        print(f"Monstro causou {dano_monstro} de dano! Vida de {aventureiro["nome"]}: {aventureiro["vida"]}")

        if aventureiro["vida"] <= 0:
            return aventureiro_esta_vivo(aventureiro)
...
def movimentar(aventureiro, direcao):
    ...
    efeito = choices(["nada", "monstro"], [0.6, 0.4])[0]
    if efeito == "monstro":
...
def gerar_tesouro():
    ...
    while x == 0 and y == 0:
        x =randint(0,9)
        y = randint(0,9)
    pos_tesouro.append(x)
    pos_tesouro.append(y)
    return(pos_tesouro)
def main():
    """
    ...
    aventureiro = {
        "forca": randint(10, 18),
        "defesa": randint(10, 18),
        "vida": randint(100, 120),
        "posicao": [0, 0]
    }
